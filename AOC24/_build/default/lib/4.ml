...
let countXMAS lines =
  ...
  let getChar x y = 
    if x < 0 || x >= maxX || y < 0 || y >= maxY then None else Some(List.nth (List.nth lines x) y) in
  let search x y =
      let rec beam a b count dir =
        if count = List.length word then 1 else
          let (dx, dy) = dir in
          let nextX = a + dx in
          let nextY = b + dy in
          if getChar nextX nextY = List.nth_opt word count then beam nextX nextY (count + 1) dir else 0
